<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        // Update data
        $query = "UPDATE pengurus SET nama = ?, alamat = ?, gender = ?, gaji = ? WHERE id = ?";
        $stmt = mysqli_prepare($conn, $query);
        mysqli_stmt_bind_param($stmt, "sssii", $nama, $alamat, $gender, $gaji, $id);
        
        if (mysqli_stmt_execute($stmt)) {
            echo json_encode(array('response' => 200, 'pesan' => 'Data berhasil Diubah'));
        } else {
            echo json_encode(array('response' => 500, 'pesan' => 'Gagal mengubah data'));
        }
